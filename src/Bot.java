    ...
    private List<int[]> getEmptySpacesHeuristic(String[][] board) {
        ...
        for (int i = 0; i < ROW; i++) {
            for (int j = 0; j < COL; j++){
                if (board[i][j].equals("")) {
                    if (i - 1 >= 0) {
                        if (isBotOrPlayer(board[i - 1][j])) { emptySpaces.add(new int[]{i, j}); }
                    } else if (i + 1 < ROW) {
                        if (isBotOrPlayer(board[i + 1][j])) { emptySpaces.add(new int[]{i, j}); }
                    } else if (j - 1 >= 0 ) {
                        if (isBotOrPlayer(board[i][j - 1])) { emptySpaces.add(new int[]{i, j}); }
                    } else if (j + 1 < COL) {
                        if (isBotOrPlayer(board[i][j + 1])) { emptySpaces.add(new int[]{i, j}); }
                    }
//                    else if (moreSpace){
...
//        return new int[]{res[3], res[4]};
    ...
    private int[] minimaxDecision(String[][] board, List<int[]> emptySpaces, int bvalue, int depth, int alpha, int beta, boolean isMaximizing) {
        if (depth == 0) {
